...
testUnit = do
  ...
  -- Should be: (BoolVal False,fromList [])
  putStrLn $ show $ WhileInterp.run (Op Gt (Val (IntVal 3)) (Val (IntVal 5)))
  -- Should be: (BoolVal True,fromList [])
  putStrLn $ show $ WhileInterp.run (Op Lt (Val (IntVal 3)) (Val (IntVal 5)))
  -- Should be: (IntVal 3,fromList [])
  putStrLn $ show $ WhileInterp.run (Op Divide (Val (IntVal 15)) (Val (IntVal 5)))
  -- Should be: (IntVal 80,fromList [])
  putStrLn $ show $ WhileInterp.run (Op Times (Val (IntVal 16)) (Val (IntVal 5)))
